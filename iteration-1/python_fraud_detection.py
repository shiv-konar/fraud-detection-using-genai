...
def run(csv_path: str, flag_threshold: int, config: Dict):
    txns = read_transactions(csv_path)

    # Build simple per-user history and seen devices as we iterate
    user_history: Dict[str, List[Transaction]] = {}
    seen_devices: Dict[str, set] = {}

    print(f"Analysing {len(txns)} transactions...\n")

    for t in txns:
        # History before current transaction
        history = user_history.get(t.user_id, [])
        devices = seen_devices.get(t.user_id, set())
        ...
        print(f"txn_id={t.txn_id} user={t.user_id} amount={t.amount:.2f} {t.currency} time={t.timestamp.isoformat()}")
        print(f"  score={score} severity={sev} status={flagged}")
        if rule_results:
            for rr in rule_results:
                print(f"  - {rr.name}: +{rr.points} ({rr.details})")
        else:
        ...
        history.append(t)
        user_history[t.user_id] = history
        devices.add(t.device_id)
        seen_devices[t.user_id] = devices

    # Simple summary
    flagged_count = 0
    for t in txns:
        # recompute quickly for summary
        pass  # already printed; could accumulate during loop if needed

def main():
    parser = argparse.ArgumentParser(description="Rule-based fraud detection CLI")
    ...
    parser.add_argument("--high-amount-threshold", type=float, default=2000.0, help="Amount threshold for HighAmount rule")
    parser.add_argument("--velocity-window-minutes", type=int, default=5, help="Window in minutes for velocity rule")
    parser.add_argument("--velocity-max-allowed", type=int, default=3, help="Max allowed transactions in window before flagging")
    parser.add_argument("--blacklisted-mcc", type=int, nargs="*", default=[4829, 7995], help="List of blacklisted MCC codes")
    args = parser.parse_args()

    config = {
        "high_amount_threshold": args.high_amount_threshold,
        "velocity_window_minutes": args.velocity_window_minutes,
        "velocity_max_allowed": args.velocity_max_allowed,
        "blacklisted_mcc": args.blacklisted_mcc,
    }

    run(args.csv, args.flag_threshold, config)
